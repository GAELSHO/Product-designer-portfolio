...
export interface DockProps {
  ...
  /**
   * AÑADIDO (no viene de unlumen-ui): lado donde aparece el tooltip.
   * El original lo dibuja siempre arriba, que es lo correcto para un dock
   * anclado abajo. Este dock va fijo en el top de la pantalla, así que
   * necesita "bottom" o el tooltip se sale del viewport.
   * @default "top"
   */
...
export function Dock({
  ...
  const [tooltipTopOffset, setTooltipTopOffset] = React.useState(0);
  ...
  React.useEffect(() => {
    ...
    const update = () => {
      ...
      if (iconEl && dockEl) {
        ...
        setTooltipTopOffset(iconRect.bottom - dockRect.top);
      }
            ...
            <motion.div
              ...
              style={
                tooltipPlacement === "bottom"
                  ? { left: tooltipX, top: tooltipTopOffset + 8, x: "-50%" }
                  : { left: tooltipX, bottom: tooltipBottomOffset + 8, x: "-50%" }
              }
              initial={{ opacity: 0, y: tooltipPlacement === "bottom" ? -6 : 6, scale: 0.94 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, y: tooltipPlacement === "bottom" ? -6 : 6, scale: 0.94 }}
            ...
            >
              {tooltipPlacement === "bottom" && (
                <svg
                  ...
                  height="4"
                  viewBox="0 0 8 4"
                  className="-mb-px text-background"
                  aria-hidden
                >
